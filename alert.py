"""
Name of Script: Surgeon Control Panel( General OT )
Date: 17-JAN-2025
...
oxygen_pin = 25  # Example GPIO pin for OXYGEN
nitrous_pin = 8  # Example GPIO pin for NITROUS
air4bar_pin = 7  # Example GPIO pin for AIR 4 BAR
air7bar_pin = 12  # Example GPIO pin for AIR 7 BAR
vacuum_pin = 16  # Example GPIO pin for VACUUM
# GPIO setup
GPIO.setmode(GPIO.BCM)  # Use Broadcom GPIO numbering
GPIO.setup(oxygen_pin, GPIO.IN)  # Set OXYGEN pin as input
GPIO.setup(nitrous_pin, GPIO.IN)  # Set NITROUS pin as input
GPIO.setup(air4bar_pin, GPIO.IN)  # Set AIR 4 BAR pin as input
GPIO.setup(air7bar_pin, GPIO.IN)  # Set AIR 7 BAR pin as input
GPIO.setup(vacuum_pin, GPIO.IN)  # Set VACUUM pin as input
...
power_icon = ctk.CTkImage(Image.open("/home/pi/application/img/off.png"), size=(60, 60))  # Ensure the path is correct
...
icon_lumi = ctk.CTkImage(Image.open("/home/pi/application/img/lumipre.png"), size=(40, 40))  
icon_pos = ctk.CTkImage(Image.open("/home/pi/application/img/pospre.png"), size=(40, 40)) 
...
# Function to toggle the UV LIGHT
enabled = False
def uv_toggle():
    global enabled
    if enabled:
...
def show_selected_time():
    ...
    if selected_hour == "Hour" or selected_minute == "Minute" or selected_second == "Second" or not selected_date:
        ...
        selected_time = f"Selected Time: {selected_hour}:{selected_minute}:{selected_second} on {selected_date}"
...
def show_popup():
    ...
    style.configure("CustomCombobox.TCombobox", 
                    ...
                    font=('Helvetica', 30),  # Increase font size for larger height
                    padding=(15, 15))  # Add padding for increased width and height
...
# Function to toggle the  state and light for surgery Light
...
def create_value_with_unit(parent, value, unit):
    ...
    value_label = ctk.CTkLabel(frame, text=value, font=("Poppins Regular", 40), text_color="#ECB67F")
    value_label.grid(row=0, column=0, padx=0)
    unit_label = ctk.CTkLabel(frame, text=unit, font=("Poppins Medium", 30), text_color="#ECB67F")
...
def update_values():
    ...
    try:
     ...
     # OXYGEN
    if GPIO.input(oxygen_pin) == GPIO.HIGH:
        alertFlag=0
        canvas_oxygen.itemconfig(circle_oxygen, fill="#409A94")  # Green for HIGH
    else:
        canvas_oxygen.itemconfig(circle_oxygen, fill="#E49157")  # Red for LOW
    
    # NITROUS
    if GPIO.input(nitrous_pin) == GPIO.HIGH:
        canvas_nitrous.itemconfig(circle_nitrous, fill="#409A94")  # Green for HIGH
    else:
        canvas_nitrous.itemconfig(circle_nitrous, fill="#E49157")  # Red for LOW
        
    # AIR 4 BAR
    if GPIO.input(air4bar_pin) == GPIO.HIGH:
        canvas_air4bar.itemconfig(circle_air4bar, fill="#409A94")  # Green for HIGH
    else:
        canvas_air4bar.itemconfig(circle_air4bar, fill="#E49157")  # Red for LOW

    # AIR 7 BAR
    if GPIO.input(air7bar_pin) == GPIO.HIGH:
        canvas_air7bar.itemconfig(circle_air7bar, fill="#409A94")  # Green for HIGH
    else:
        canvas_air7bar.itemconfig(circle_air7bar, fill="#E49157")  # Red for LOW

    # VACUUM
    if GPIO.input(vacuum_pin) == GPIO.HIGH:
        canvas_vacuum.itemconfig(circle_vacuum, fill="#409A94")  # Green for HIGH
    else:
        canvas_vacuum.itemconfig(circle_vacuum, fill="#E49157")  # Red for LOW
...
def draw_clock_numbers():
    for i in range(1, 13):
        ...
        canvas.create_text(x, y, text=str(i), font=("Helvetica", 10, 'bold'), fill="WHITE")
...
app.grid_columnconfigure(0, minsize=220, weight=1)  # Set a minimum width for column 0
app.grid_columnconfigure(1, minsize=220, weight=1)  # Set a minimum width for column 1
app.grid_columnconfigure(2, minsize=220, weight=1)  # Set a minimum width for column 2
...
logo_label.pack(pady=10)  # Use pack for the logo

# Second cell: Website name
website_label = ctk.CTkLabel(website_frame, text="www.microfilt.com", font=("Poppins Light", 19), text_color="#3D3C3C", fg_color='#FFFFFF')
...
titleLabel = ctk.CTkLabel(dashtitle, text="Surgeon Control Panel", font=("Poppins SemiBold", 34), text_color="#3D3C3C", fg_color='#E7E7E7')
...
center_frame21.pack(pady=5,padx=113)

center_frame22 = ctk.CTkFrame(top_frame, fg_color='#F3F1F1')
center_frame22.pack(pady=5)

# Create two vertical frames (left and right) inside the main frame
left_frame = ctk.CTkFrame(bottom_frame,fg_color="#F3F1F1")
left_frame.grid(row=0, column=0,padx=(20,10), pady=20) 

right_frame = ctk.CTkFrame(bottom_frame,fg_color="#ECB67F")
right_frame.grid(row=0, column=1,padx=(10,20), pady=20) 


######################################################### SURGERY LIGHTING Frame Content Added ###############################################################

# Create Heading Label
sg_label = ctk.CTkLabel(surgery_lighting, text="SURGERY LIGHTING", font=("Poppins Medium", 23), text_color="#3D3C3C")
sg_label.pack(pady=(20,10))
...
for index, frame in enumerate(subSurframesArr):
    ...
    surLightSlider.set(0)  # Initialize the slider to zero position
...
OP_label = ctk.CTkLabel(center_frame21, text="OPERATION TIME", font=("Poppins Medium", 23), text_color="#3D3C3C")
OP_label.pack(pady=(10, 5))

# Create time display label
time_label = ctk.CTkLabel(center_frame21, text="00:00:00", font=("Arial", 70),text_color="#ECB67F")
...
stop_resume_button = ctk.CTkButton(center_frame22, image=start_image, fg_color="#F3F1F1", text="", width=40, height=40,
                                             border_color="#409A94", border_width=2,  hover_color="#F3F1F1",command=toggle)
...
restart_button = ctk.CTkButton(center_frame22, image=restart_image, fg_color="#F3F1F1", text="", width=40, height=40,
                                            border_color="#409A94", border_width=2, hover_color="#F3F1F1", command=reset_timer)
...
canvas = tk.Canvas(left_frame, width=180, height=180, bg="#ffffff", highlightthickness=0)
canvas.grid(row=0, column=0)

# Clock center, radius, and hand lengths
center_x, center_y = 90, 90  # Adjusted center for larger canvas
radius = 80
...
canvas.create_oval(88, 88, 92, 92, outline="white", width=2, fill="white") 

# Bind the click event to the canvas
canvas.bind("<Button-1>", lambda event: show_popup())

# Create and configure the first label
day_label = ctk.CTkLabel(right_frame, text="", font=("Poppins SemiBold ", 34), text_color="WHITE")
day_label.grid(row=0, column=0, padx=25, pady=(15, 0), sticky="nsew")  # Top label

# Create and configure the second label
date_label = ctk.CTkLabel(right_frame, text="", font=("Poppins SemiBold ", 34), text_color="WHITE")
...
amb_label = ctk.CTkLabel(ambient_lighting, text="AMBIENT LIGHTING", font=("Poppins Medium", 23), text_color="#3D3C3C")
amb_label.pack(pady=(20,10))
...
for index, frame in enumerate(subAmbframesArr):
    ...
    amb_toggle_button = ctk.CTkButton(frame, text="", image=disabled_image,  
                                   border_width=0, width=40, height=40,
                                   fg_color="#F3F1F1", hover_color="#F3F1F1")
    amb_toggle_button.pack(side='left', padx=(0, 20))  
    ...
    amb_light_slider.pack(side='left') 
...
main3row.grid(row=4, column=0, columnspan=3, padx=(60,60), pady=10, sticky='nsew')

center_frame31 = ctk.CTkFrame(main3row, fg_color='#FFFFFF')
center_frame31.grid(row=1,column=0,pady=5,padx=28)

center_frame32 = ctk.CTkFrame(main3row, fg_color='#FFFFFF')
center_frame32.grid(row=1,column=1,pady=5,padx=28)

center_frame33 = ctk.CTkFrame(main3row, fg_color='#FFFFFF')
center_frame33.grid(row=1,column=2,pady=5,padx=28)

center_frame34 = ctk.CTkFrame(main3row, fg_color='#FFFFFF')
center_frame34.grid(row=1,column=3,pady=5,padx=28)

center_frame35 = ctk.CTkFrame(main3row, fg_color='#FFFFFF')
center_frame35.grid(row=1,column=4,pady=15,padx=28)
...
header_label = ctk.CTkLabel(main3row, text="OPERATION THEATER ENVIRONMENT", font=("Poppins SemiBold", 22), text_color="#2b2b2b")
header_label.grid(row=0, column=0, padx=23, pady=(17,13), columnspan=5)

##########################################################################################################################################################

first_line_frame = ctk.CTkFrame(center_frame31,fg_color='#FFFFFF')
first_line_frame.pack(padx=60, pady=(10,5))

# Create the icon label
icon_label_temp = ctk.CTkLabel(first_line_frame, text="", image=icon_temp)
icon_label_temp.pack(side='left',padx=(0,10))  
...
second_line_frame.pack(padx=0, pady=(0,5))
...
first_line_frame.pack(padx=80, pady=(10,5))
# Create the icon label
...
humidity_label.pack(side='left',padx=(10,0))  
...
humidity_value_with_unit.pack(padx=10, pady=10)
...
first_line_frame.pack(padx=80, pady=15)
...
second_line_frame.pack(padx=0, pady=(0,15))
...
positive_label.pack(side='left',padx=(10,0))  
...
##################################################################Create Frame for row 4 and Five Subframe For GAS #################################################################

# Create a frame for each cell with a background color
main4row = ctk.CTkFrame(app, fg_color='#F3F1F1')
main4row.grid(row=5, column=0, columnspan=3, padx=(60,60), pady=10, sticky='ew')
...
# Add header label for Operation Theater Environment
header_label = ctk.CTkLabel(main4row, text="MEDICAL GAS ALARM", font=("Poppins SemiBold", 22), text_color="#2b2b2b")
header_label.grid(row=0, column=0, columnspan=5, pady=(10,0), sticky='ew')


##########################################################################################################################################################

# Create a frame to center the content
center_frame41 = ctk.CTkFrame(main4row, fg_color='#FFFFFF')
center_frame41.grid(row=1,column=0,pady=13,padx=(55,20))

gasLabel_oxygen = ctk.CTkLabel(center_frame41, text="OXYGEN", font=("Poppins Medium", 18), text_color="#3D3C3C")
gasLabel_oxygen.pack(side='left', pady=10, padx=(90,50))

canvas_oxygen = tk.Canvas(center_frame41, width=50, height=50, bg="white", highlightthickness=0)
canvas_oxygen.pack(side=tk.LEFT,pady=10, padx=(10,40))
circle_oxygen = canvas_oxygen.create_oval(5, 5, 45, 45, fill="#ECB67F", outline="#FFFFFF")  # Draw a circle


##########################################################################################################################################################

# Create a frame to center the content
center_frame42 = ctk.CTkFrame(main4row, fg_color='#FFFFFF')
center_frame42.grid(row=1,column=1,pady=13,padx=20)

gasLabel_nitrous = ctk.CTkLabel(center_frame42, text="NITROUS", font=("Poppins Medium", 18), text_color="#3D3C3C")
gasLabel_nitrous.pack(side='left', pady=10, padx=(90,50))

canvas_nitrous = tk.Canvas(center_frame42, width=50, height=50, bg="white", highlightthickness=0)
canvas_nitrous.pack(side=tk.LEFT,pady=10, padx=(10,40))
circle_nitrous = canvas_nitrous.create_oval(5, 5, 45, 45, fill="#ECB67F", outline="#FFFFFF")  # Draw a circle


##########################################################################################################################################################

# Create a frame to center the content
center_frame43 = ctk.CTkFrame(main4row, fg_color='#FFFFFF')
center_frame43.grid(row=1,column=2,pady=13,padx=20)

gasLabel_air4bar = ctk.CTkLabel(center_frame43, text="AIR 4 BAR", font=("Poppins Medium", 18), text_color="#3D3C3C")
gasLabel_air4bar.pack(side='left', pady=10, padx=(90,40))

canvas_air4bar = tk.Canvas(center_frame43, width=50, height=50, bg="white", highlightthickness=0)
canvas_air4bar.pack(side=tk.LEFT,pady=10, padx=(10,30))
circle_air4bar = canvas_air4bar.create_oval(5, 5, 45, 45, fill="#ECB67F", outline="#FFFFFF")  # Draw a circle


##########################################################################################################################################################

# Create a frame to center the content
center_frame44 = ctk.CTkFrame(main4row, fg_color='#FFFFFF')
center_frame44.grid(row=1,column=3,pady=13,padx=20)

gasLabel_air7bar = ctk.CTkLabel(center_frame44, text="AIR 7 BAR", font=("Poppins Medium",18), text_color="#3D3C3C")
gasLabel_air7bar.pack(side='left', pady=10,  padx=(80,40))

canvas_air7bar = tk.Canvas(center_frame44, width=50, height=50, bg="white", highlightthickness=0)
canvas_air7bar.pack(side=tk.LEFT,pady=10, padx=(10,30))
circle_air7bar = canvas_air7bar.create_oval(5, 5, 45, 45, fill="#ECB67F", outline="#FFFFFF")  # Draw a circle


##########################################################################################################################################################

# Create a frame to center the content
center_frame45 = ctk.CTkFrame(main4row, fg_color='#FFFFFF')
center_frame45.grid(row=1,column=4,pady=13,padx=20)

gasLabel_vacuum = ctk.CTkLabel(center_frame45, text="VACUUM", font=("Poppins Medium", 18), text_color="#3D3C3C")
gasLabel_vacuum.pack(side='left', pady=10,  padx=(90,50))

canvas_vacuum = tk.Canvas(center_frame45, width=50, height=50, bg="white", highlightthickness=0)
canvas_vacuum.pack(side=tk.LEFT,pady=10, padx=(10,30))
circle_vacuum = canvas_vacuum.create_oval(5, 5, 45, 45, fill="#409A94", outline="#FFFFFF")  # Draw a circle
